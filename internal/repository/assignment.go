...
func createAssignment(ctx context.Context, q sqler, assignment domain.Assignment) error {
	...
	_, err := q.Exec(ctx, `INSERT INTO assignments(id,mission_id,drone_id,operator_id,status,starts_at,ends_at) VALUES ($1,$2,$3,$4,$5,$6,$7)`, assignment.ID, assignment.MissionID, assignment.DroneUnitID, assignment.FleetOperatorID, assignment.Status, assignment.StartsAt, assignment.EndsAt)
...
func NewAssignment(missionID, droneID, operatorID string, startsAt, endsAt time.Time) domain.Assignment {
	return domain.Assignment{ID: uuid.NewString(), MissionID: missionID, DroneUnitID: droneID, FleetOperatorID: operatorID, StartsAt: startsAt, EndsAt: endsAt, Status: "queued"}
}
