...
func (p *Postgres) QueryAudit(ctx context.Context, filter audit.Filter, from, to time.Time, limit, offset int) ([]audit.Event, int, error) {
	...
	rows, err := p.pool.Query(ctx, fmt.Sprintf(`SELECT request_id,operator_id,object_type,object_id,action,outcome,detail,created_at FROM audit_events %s ORDER BY created_at DESC LIMIT $3 OFFSET $4`, where), args...)
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.RequestID, &item.FleetOperatorID, &item.ObjectType, &item.ObjectID, &item.Action, &item.Outcome, &item.Detail, &item.CreatedAt); err != nil {
			return nil, 0, err
		}
