...
func (e *perJobExecutor) Execute(_ context.Context, job DroneMissionBatchJob) error {
	e.mu.Lock()
	defer e.mu.Unlock()
	e.calls[job.ID]++
...
func TestDroneMissionBatchAttemptLimitDoesNotAffectTheNextJob(t *testing.T) {
	executor := &perJobExecutor{calls: map[string]int{}}
	processor := NewDroneMissionBatchProcessor(executor, RetryPolicy{Attempts: 3, BaseDelay: time.Millisecond, MaxDelay: time.Millisecond}, nil, nil)
	if err := processor.Process(t.Context(), DroneMissionBatchJob{ID: "short", MaxAttempts: 1}); err == nil {
		t.Fatal("one-attempt job unexpectedly succeeded")
	}
	if err := processor.Process(t.Context(), DroneMissionBatchJob{ID: "long"}); err != nil {
		t.Fatal(err)
	}
