...
func (f *fakeExecutor) Execute(context.Context, DroneMissionBatchJob) error {
	f.attempts++
...
func TestDroneMissionBatchProcessorRecordsSuccessAfterRetry(t *testing.T) {
	executor := &fakeExecutor{fail: 1}
	metrics := &Metrics{}
	processor := NewDroneMissionBatchProcessor(executor, RetryPolicy{Attempts: 3, BaseDelay: time.Nanosecond, MaxDelay: time.Nanosecond}, nil, metrics)
	if err := processor.Process(context.Background(), DroneMissionBatchJob{ID: "missionBatch-1"}); err != nil {
		t.Fatal(err)
	}
...
func TestDroneMissionBatchProcessorRecordsPermanentFailure(t *testing.T) {
	executor := &fakeExecutor{fail: 10}
	metrics := &Metrics{}
	processor := NewDroneMissionBatchProcessor(executor, RetryPolicy{Attempts: 2, BaseDelay: time.Nanosecond, MaxDelay: time.Nanosecond}, nil, metrics)
	if err := processor.Process(context.Background(), DroneMissionBatchJob{ID: "missionBatch-2"}); err == nil {
