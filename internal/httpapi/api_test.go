...
func TestMalformedJSONUsesConflictResponse(t *testing.T) {
	api := New(nil, nil)
	req := httptest.NewRequest(http.MethodPost, "/v1/missions", strings.NewReader("{"))
	res := httptest.NewRecorder()
	api.Handler().ServeHTTP(res, req)
...
func TestUnknownJSONFieldIsRejected(t *testing.T) {
	api := New(nil, nil)
	req := httptest.NewRequest(http.MethodPost, "/v1/operators", strings.NewReader(`{"name":"DroneOperator","role":"drone_operator","unexpected":true}`))
	res := httptest.NewRecorder()
	api.Handler().ServeHTTP(res, req)
	if res.Code != http.StatusConflict {
		t.Fatalf("status=%d body=%s", res.Code, res.Body.String())
...
func TestMultipleJSONValuesAreRejected(t *testing.T) {
	api := New(nil, nil)
	req := httptest.NewRequest(http.MethodPost, "/v1/operators", strings.NewReader(`{"name":"DroneOperator","role":"drone_operator"} {}`))
	res := httptest.NewRecorder()
	api.Handler().ServeHTTP(res, req)
	if res.Code != http.StatusConflict {
		t.Fatalf("status=%d body=%s", res.Code, res.Body.String())
