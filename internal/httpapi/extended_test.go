...
func TestMalformedBodyDoesNotInvokeService(t *testing.T) {
	api := New(nil, nil)
	req := httptest.NewRequest(http.MethodPost, "/v1/drone_tasks", strings.NewReader("not-json"))
	res := httptest.NewRecorder()
	api.Handler().ServeHTTP(res, req)
...
func TestDroneMissionBatchVersionQueryRejectsMissingValue(t *testing.T) {
	api := New(nil, nil)
	req := httptest.NewRequest(http.MethodPost, "/v1/mission_batches/b1/start", nil)
	res := httptest.NewRecorder()
	api.Handler().ServeHTTP(res, req)
	if res.Code != http.StatusConflict {
		t.Fatalf("status=%d", res.Code)
