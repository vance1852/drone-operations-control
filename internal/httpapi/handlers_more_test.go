...
func TestRequiredHeader(t *testing.T) {
	...
	if _, err := requiredHeader(req, "X-FleetOperator-ID"); err == nil {
		t.Fatal("missing header accepted")
	}
	req.Header.Set("X-FleetOperator-ID", "op-1")
	if value, err := requiredHeader(req, "X-FleetOperator-ID"); err != nil || value != "op-1" {
		t.Fatalf("value=%s err=%v", value, err)
	}
}

func TestFleetOperatorCreateMalformedBody(t *testing.T) {
	api := New(nil, nil)
	req := httptest.NewRequest(http.MethodPost, "/v1/operators", strings.NewReader("{"))
