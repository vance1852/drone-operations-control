...
func DefaultMissionMachine() StateMachine {
	return NewStateMachine(map[string][]string{"draft": {"scheduled"}, "scheduled": {"active", "closed"}, "active": {"closed"}})
}

func DefaultDroneTaskMachine() StateMachine {
	return NewStateMachine(map[string][]string{"queued": {"completed"}, "completed": {"device_transfer_pending"}, "device_transfer_pending": {"accepted"}, "accepted": {"in_progress"}, "in_progress": {"verified", "rejected"}, "verified": {"archived"}, "rejected": {"archived"}})
}

func DefaultDroneMissionBatchMachine() StateMachine {
