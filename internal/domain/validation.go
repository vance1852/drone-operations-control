...
func ValidateObservation(riskScore, alertThreshold float64, scale string) error {
	if math.IsNaN(riskScore) || math.IsNaN(alertThreshold) || math.IsInf(riskScore, 0) || math.IsInf(alertThreshold, 0) || riskScore < 0 || alertThreshold < 0 {
		return fmt.Errorf("telemetry scores cannot be negative: %w", ErrConflict)
	}
	if strings.TrimSpace(scale) == "" {
		return fmt.Errorf("telemetry scale is required: %w", ErrConflict)
	}
	return nil
}
