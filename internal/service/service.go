...
func (s *Service) ReviewObservation(ctx context.Context, meta RequestMeta, telemetryID, taskID string, accepted bool, telemetryVersion, taskVersion int64) error {
	var safetyAlertID string
	if !accepted {
		var err error
		safetyAlertID, err = s.createReviewAlert(ctx, taskID)
		if err != nil {
			return err
		}
	}
	return s.repo.InTx(ctx, func(tx repository.Repository) error {
		...
		if !accepted {
			if err := tx.WriteAudit(ctx, audit(meta, "safety_alert", safetyAlertID, "open", "success", nil)); err != nil {
				return err
			}
		}
