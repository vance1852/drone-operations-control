...
import (
	...
	auditpkg "drone-operations-control/internal/audit"
...
func auditInput(event auditpkg.Event, detail []byte) repository.AuditInput {
	return repository.AuditInput{RequestID: event.RequestID, FleetOperatorID: event.FleetOperatorID, ObjectType: event.ObjectType, ObjectID: event.ObjectID, Action: event.Action, Outcome: event.Outcome, Detail: detail}
}
